...
asg_common <- function(y, x, count, group, priors, niter=2000, nchains=3, nclusters=nchains, burnin=niter/2, thin=10, inits=NULL, mle=FALSE){
  # Load Library
  require(R2jags)
  ...
  if(isTRUE(mle)){
    ...
    c1 <- c2 <- c3 <- rep(NA, 6)
      ...
      if("try-error" %in% class(t)){
        tt <- try(optim(par=c(-5, -5, 12, -2.5, 1, 10), log.lik, y=suby, x=subx, num=subnum, hessian=TRUE))
        if("try-error" %in% class(tt)){
          fit <- optim(par=c(-5, -5, 12, -2.5, 1, 2), log.lik, y=suby, x=subx, num=subnum, hessian=TRUE)
        }
        else{
          fit <- optim(par=c(-5, -5, 12, -2.5, 1, 10), log.lik, y=suby, x=subx, num=subnum, hessian=TRUE)
        }
      }else{
  ...
  m = jags(data=dat, 
                    ...
                    model.file = system.file("model", "asg_common.txt", package = "functform"),
                    n.chains = nchains, 
                    n.iter = niter,
                    n.burnin=burnin,
                    n.thin=thin
