...
#'
#' @examples
#' mean.vector = c(beta1, eta, mu, lsigma1, lsigma2)
#' 
#' priors <- list()
#' priors$mu0  # A 5x1 vector
#' priors$C    # A 5x5 precision matrix for mean MVN
#' priors$phi0 # A 5x5 precision matrix for Wish Dist
#' 
#'
#' @export

asg_multi5 <- function(y, x, count, group, priors, niter=2000, nchains=3, nclusters=nchains, burnin=niter/2, thin=10){
  # Load Library
  require(R2jags)
  ...
  m = jags.parallel(data=dat, 
                    ...
                    model.file = system.file("model", "multi_hier5.txt", package = "functform"),
                    n.chains = nchains, 
                    n.iter = niter,
                    n.burnin=burnin,
                    n.thin=thin,
