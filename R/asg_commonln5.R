#' asg_commonln5
#' 
#' Runs Asymmetric Gaussian MCMC with a common mean structure accross the groups with log-normal priors with one intercept
#'
...
asg_commonln5 <- function(y, x, count, group, priors, niter=2000, nchains=3, nclusters=nchains, burnin=niter/2, thin=10, inits=NULL, mle=FALSE){
  # Load Library
  require(R2jags)
  ...
  m = jags(data=dat, 
                    ...
                    model.file = system.file("model", "asg_commonln5.txt", package = "functform"),
                    n.chains = nchains, 
                    n.iter = niter,
                    n.burnin=burnin,
                    n.thin=thin
