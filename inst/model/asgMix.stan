...
model {
  ...
  for(s in 1:nS){
    tau2[s] ~ student_t(4, 0, 1); //Prior on Season SD
  }
  ...
  for(g in 1:nG){
    ...
    for(s in 1:nS){
      row(ctheta[g], s) ~ multi_normal_cholesky(mu_theta[g], diag_pre_multiply(tau2[s], Lcorr)); // c(beta1[g], beta2[g], leta[g], mu[g], lsigma1[g], lsigma2[g])
    }
  }
