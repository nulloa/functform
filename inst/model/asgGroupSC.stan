...
parameters {
  ...
  vector<lower=0>[k] tau2[nS];
  cholesky_factor_corr[k] Lcorr;
  vector[k] mu_theta[nS]; // Means for Parameters; This is nS x K matrix
  vector[k] mu_g;
}
...
model {
  ...
  for(s in 1:nS){
    tau2[s] ~ student_t(4, 0, 1); //Prior on group SD
    mu_theta[s] ~ multi_normal_cholesky(mu_g, diag_pre_multiply(tau, Lcorr));
    for(g in 1:nG){
      row(ctheta[g], s) ~ multi_normal_cholesky(mu_theta[s], diag_pre_multiply(tau2[s], Lcorr)); // c(beta1[g], beta2[g], leta[g], mu[g], lsigma1[g], lsigma2[g])
    }
  }
