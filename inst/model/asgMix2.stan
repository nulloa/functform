...
parameters {
  ...
  vector[k] mu_theta[nS]; // Means for Parameters; This is nG x K matrix
  vector[k] mu_g;
}
...
model {
  ...
  for(g in 1:nG){
    tau2[g] ~ student_t(4, 0, 1); //Prior on Season SD
  }
  ...
  for(s in 1:nS){
    ...
    for(g in 1:nG){
      row(ctheta[g], s) ~ multi_normal_cholesky(mu_theta[s], diag_pre_multiply(tau2[g], Lcorr)); // c(beta1[g], beta2[g], leta[g], mu[g], lsigma1[g], lsigma2[g])
    }
  }
